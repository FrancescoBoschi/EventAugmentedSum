...
class AbsSelfCriticalPipeline(object):
    ...
    def train_step(self, sample_time=1):
        ...
        for i in range(self._accumulate_g_step):
            ...
            # by default we don't into consideration the L_seq loss
...
class MultiTaskPipeline(object):
    ...
    def train_step(self):
        ...
        # losses[0] is L_seq obtained using the nll_loss(logit, target) with logit of siz
        #           e.g. (190, 50265) and target of size e.g. (190,)
        # losses[1:] refer to L_mask and are computed with binary_cross_entropy, by
        #            default len(losses[1:]) = 1
        loss = losses[0]
