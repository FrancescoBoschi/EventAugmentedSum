...
def prepro_fn_copy_bert(tokenizer, max_src_len, max_tgt_len, batch):
    ...
    def prepro_one(sample):
        ...
        return source, target, raw_sentences, article_id
...
def convert_batch_copy_bert(tokenizer, max_src_len, batch):
    ...
    sources, targets, raw_sentences, article_ids = map(list, unzip(batch))
    ...
    batch = [sources, list(zip(src_exts, tar_ins, targets, src_length)), raw_sentences, article_ids]
    return batch
