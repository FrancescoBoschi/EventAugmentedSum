...
import pickle
from graph_augmented_sum.data.batcher import pad_batch_tensorize, pad_batch_tensorize_3d
from graph_augmented_sum.data.batcher import make_adj_triple, make_adj, make_adj_edge_in
from graph_augmented_sum.data.RLbatcher import process_nodes, process_subgraphs, process_nodes_bert
from graph_augmented_sum.data.ExtractBatcher import subgraph_make_adj_edge_in, subgraph_make_adj
...
def prepro_fn_gat(max_src_len, max_tgt_len, batch, node_max_len=30, key='summary_worthy', adj_type='edge_as_node', docgraph=True):
    ...
    tuple[1]: list where the full text of the abstract is stored
    tuple[2]: dictionary containing all the pruned nodes
    tuple[3]: dictionary containing all the pruned edges
    tuple[4]: list where each <subject, predicate, object> triplet is stored
              in the corresponding paragraph indicating also the corresponding sentence
    tuple[5]: list expressing to which paragraph each sentence belongs
    """

    def prepro_one(sample):
        # sample corresponds to batch[i]

        source, target, nodes, edges, subgraphs, paras = sample

        # each sentence is splitted in a list of all its words
        source_sent = [sent.strip().lower().split() for sent in source]

        # list of all tokens in the document, if there are more than 1024
        # words, we remove the exceeding ones
        source = ' '.join(source).strip().lower().split()[:max_src_len]

        # the same is done for the abstract, but in this case the max
        # number of words is 150
        ...
        # nodes that don't contain words won't be considered
        ...
        # from a dictionary to a list of tuples, where tuple[0] is the node key e.g node_0
        # and tuple[1] contains all the relevant information corresponding to the node
        ...
        for _id, content in sorted_nodes:
            if _id not in oor_nodes:
                ...
                words = [pos for mention in content['content'] for pos in mention['word_pos'] if pos != -1]
                words = [word for word in words if word < max_len]
                # if a node text contains more than 30 words the exceeding words are removed
        ...
        # number of words in each node considering the aliases
        nodelength = [len(words) for words in nodewords]
        # frequency of each node, considering that each node text can be expressed
        # in different ways e.g. 'the players' can be expressed as 'they', 'them' ecc.
        ...
        sum_worthy_edges = []
        id2edge = {}
        # from a dictionary to a list of tuples, where tuple[0] is the edge key e.g node_0
        # and tuple[1] contains all the relevant information corresponding to the edge
...
def prepro_fn_gat_bert(tokenizer, align, max_src_len, max_tgt_len, batch, node_max_len=30,
    ...
    """
        ...
        We consider tuple = batch[i] the tuple corresponding to the i-th document
        tuple[0]: list of all sentences in the article
        tuple[1]: list of the sentences in the abstract
        tuple[2]: dictionary containing all the pruned nodes
        tuple[3]: dictionary containing all the pruned edges
        tuple[4]: list where each <subject, predicate, object> triplet is stored
                  in the corresponding paragraph indicating also the corresponding sentence
    ...
    def prepro_one(sample):
        ...
        # each sentence is splitted in a list of all its tokens
        ...
        # number of tokens in each node considering the aliases e.g a node can be expressed as
        # 'the players' and 'they', in this case the node length is 3
        nodelength = [len(words) for words in nodewords]

        # frequency of each node, considering that each node text can be expressed
        # in different ways e.g. 'the players' can be expressed as 'they' in this case the
        # node frequency is equal to 2
        nodefreq = [freq if freq < MAX_FREQ - 1 else MAX_FREQ - 1 for freq in nodefreq]
        ...
        # we consider edges as nodes, so if we have 18 nodes and 13 relations
        # we end upo with e.g len(nodewords) = 31
        if adj_type == 'edge_as_node':
            nodewords = nodewords + relations
            nodelength = nodelength + rlength
            nodefreq = nodefreq + edge_freq
...
def batchify_fn_gat_bert(tokenizer, data, cuda=True,
    ...
    """
    data[0]: is a list of lists of maximum length 512. In each list we have the token indexes composing the full document.
            Each list overlaps the previous one, so that when we input the tokens to ROBERTA we don't break a speech by splitting
            the full document into list of tokens of maximum length 512
    data[1]: contains the output of prepro_fn_gat_bert()
    """
    ...
    # adding start and end token to abstract
    ...
    # padding all the elements, the pad id is 1 and transforming them to tensors
    ...
    # padding edges so that length of the tokens ids list is equal to the maximum number
    # of tokens in an edge e.g. 20
    _relations = pad_batch_tensorize_3d(relations, pad=0, cuda=cuda)

    # masks for ignoring the tokens with id=1, those that are padded
